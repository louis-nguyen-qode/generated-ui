// id="170:23908"
import React, { CSSProperties } from "react"
import { Flex } from "antd"
import { Icon } from "@designSystem/icon"

import { QodeCustom17023783 } from "@designSystem/custom/QodeCustom17023783"

export interface QodeCustom17023908Props {
  items?: "2" | "3" | "4" | "5" | "6"
}

export const QodeCustom17023908: React.FC<
  QodeCustom17023908Props & { style?: CSSProperties }
> = ({ items = "2", style, ...rest }) => {
  return (
    <>
      {/* name="Select/Menu" id="170:23908" type="COMPONENT_SET" */}
      {/* id="170:23907" */}
      {`${items}` === `2` && (
        <>
          {/* name="items=2" id="170:23907" type="COMPONENT" */}
          <div
            style={{
              ...{
                display: "flex",
                width: "160px",
                padding: "var(--size-padding-paddingxxs, 4px)",
                flexDirection: "column",
                alignItems: "flex-start",
                borderRadius: "var(--border-radius-base, 8px)",
                background: "var(--background-bg-white, #fff)",
                boxShadow:
                  "0px 9px 28px 8px rgba(0, 0, 0, 0.05), 0px 3px 6px -4px rgba(0, 0, 0, 0.12)",
              },
              ...style,
            }}
          >
            {/* name="🧬 item 1" id="170:23900" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
            {/* name="🧬 item 2" id="170:23902" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
          </div>
        </>
      )}
      {/* id="170:23916" */}
      {`${items}` === `3` && (
        <>
          {/* name="items=3" id="170:23916" type="COMPONENT" */}
          <div
            style={{
              ...{
                display: "flex",
                width: "160px",
                padding: "var(--size-padding-paddingxxs, 4px)",
                flexDirection: "column",
                alignItems: "flex-start",
                borderRadius: "var(--border-radius-base, 8px)",
                background: "var(--background-bg-white, #fff)",
                boxShadow:
                  "0px 9px 28px 8px rgba(0, 0, 0, 0.05), 0px 3px 6px -4px rgba(0, 0, 0, 0.12)",
              },
              ...style,
            }}
          >
            {/* name="🧬 item 1" id="170:23917" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
            {/* name="🧬 item 2" id="170:23918" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
            {/* name="🧬 item 3" id="170:23921" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
          </div>
        </>
      )}
      {/* id="170:23923" */}
      {`${items}` === `4` && (
        <>
          {/* name="items=4" id="170:23923" type="COMPONENT" */}
          <div
            style={{
              ...{
                display: "flex",
                width: "160px",
                padding: "var(--size-padding-paddingxxs, 4px)",
                flexDirection: "column",
                alignItems: "flex-start",
                borderRadius: "var(--border-radius-base, 8px)",
                background: "var(--background-bg-white, #fff)",
                boxShadow:
                  "0px 9px 28px 8px rgba(0, 0, 0, 0.05), 0px 3px 6px -4px rgba(0, 0, 0, 0.12)",
              },
              ...style,
            }}
          >
            {/* name="🧬 item 1" id="170:23924" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
            {/* name="🧬 item 2" id="170:23925" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
            {/* name="🧬 item 3" id="170:23926" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
            {/* name="🧬 item 4" id="170:23930" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
          </div>
        </>
      )}
      {/* id="170:23932" */}
      {`${items}` === `5` && (
        <>
          {/* name="items=5" id="170:23932" type="COMPONENT" */}
          <div
            style={{
              ...{
                display: "flex",
                width: "160px",
                padding: "var(--size-padding-paddingxxs, 4px)",
                flexDirection: "column",
                alignItems: "flex-start",
                borderRadius: "var(--border-radius-base, 8px)",
                background: "var(--background-bg-white, #fff)",
                boxShadow:
                  "0px 9px 28px 8px rgba(0, 0, 0, 0.05), 0px 3px 6px -4px rgba(0, 0, 0, 0.12)",
              },
              ...style,
            }}
          >
            {/* name="🧬 item 1" id="170:23933" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
            {/* name="🧬 item 2" id="170:23934" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
            {/* name="🧬 item 3" id="170:23935" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
            {/* name="🧬 item 4" id="170:23936" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
            {/* name="🧬 item 5" id="170:23941" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
          </div>
        </>
      )}
      {/* id="170:23943" */}
      {`${items}` === `6` && (
        <>
          {/* name="items=6" id="170:23943" type="COMPONENT" */}
          <div
            style={{
              ...{
                display: "flex",
                width: "160px",
                padding: "var(--size-padding-paddingxxs, 4px)",
                flexDirection: "column",
                alignItems: "flex-start",
                borderRadius: "var(--border-radius-base, 8px)",
                background: "var(--background-bg-white, #fff)",
                boxShadow:
                  "0px 9px 28px 8px rgba(0, 0, 0, 0.05), 0px 3px 6px -4px rgba(0, 0, 0, 0.12)",
              },
              ...style,
            }}
          >
            {/* name="🧬 item 1" id="170:23944" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
            {/* name="🧬 item 2" id="170:23945" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
            {/* name="🧬 item 3" id="170:23946" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
            {/* name="🧬 item 4" id="170:23947" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
            {/* name="🧬 item 5" id="170:23948" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
            {/* name="🧬 item 6" id="170:23954" type="INSTANCE" */}
            <QodeCustom17023783
              style={{
                ...{
                  display: "flex",
                  height: "var(--size-height-controlheight, 32px)",
                  padding: "0px var(--size-padding-paddingsm, 12px)",
                  justifyContent: "center",
                  alignItems: "center",
                  gap: "var(--size-padding-paddingxs, 8px)",
                  alignSelf: "stretch",
                  borderRadius: "var(--style-radius-borderradius, 8px)",
                },
                ...{},
              }}
              {...{ title: "Select menu item", state: "default" }}
            />
          </div>
        </>
      )}
    </>
  )
}
